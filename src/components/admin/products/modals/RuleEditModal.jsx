...
export default function RuleEditModal({ open, onClose, rule, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await fetchJSON(`${API_RULES}/${rule.id}`, { method: 'PUT', body: JSON.stringify(payload) })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={onClose}>
      <div className="w-full max-w-lg rounded-2xl bg-[#0f1621] border border-white/10 text-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          <h3 className="text-lg font-semibold">Edit Margin Rule</h3>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {isStyleRule && (
              <div className="space-y-1 col-span-2">
                <label className="text-sm text-white/70 flex items and gap-1"><Layers className="w-4 h-4" /> Style Number</label>
                <select
                  value={form.style_no ?? ''}
